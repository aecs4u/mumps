!
! Dense BLAS benchmark for SGEMM (single precision).
...
program sgemm_bench
  ...
  seconds = t1 - t0
  gflops = (2.0d0 * dble(m) * dble(n) * dble(k) * dble(iters)) / (seconds * 1.0d9)

  write(*,'(a,i0)') 'SGEMM_BENCH_M=', m
  write(*,'(a,i0)') 'SGEMM_BENCH_N=', n
  write(*,'(a,i0)') 'SGEMM_BENCH_K=', k
  write(*,'(a,i0)') 'SGEMM_BENCH_ITERS=', iters
  write(*,'(a,f0.6)') 'SGEMM_BENCH_SECONDS=', seconds
  write(*,'(a,f0.3)') 'SGEMM_BENCH_GFLOPS=', gflops

  deallocate(a, b, c)
end program sgemm_bench
...
subroutine print_usage()
  implicit none
  write(*,'(a)') 'Usage: ./sgemm_bench [--m M] [--n N] [--k K] [--iters I]'
