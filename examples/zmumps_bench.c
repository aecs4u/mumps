...
static int load_coo_file(const char *path, MUMPS_INT *out_n, MUMPS_INT8 *out_nnz,
                        MUMPS_INT **out_irn, MUMPS_INT **out_jcn, ZMUMPS_COMPLEX **out_a) {
  FILE *fp;
  char line[1024];
  int n_int, nnz_int;
  MUMPS_INT n;
  ...
  a = (ZMUMPS_COMPLEX *)malloc((size_t)nnz * sizeof(ZMUMPS_COMPLEX));
  if (!irn || !jcn || !a) {
...
int main(int argc, char **argv) {
  ...
  if (matrix_file) {
    ...
    if (myid == 0) {
      printf("ZMUMPS_BENCH_MATRIX_FILE=%s\n", matrix_file);
    }
  } else {
